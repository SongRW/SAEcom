...
test.describe('transform-object 节点', () => {
  test('动态增删键与端口', async ({ page }) => {
    ...
    await clickReady(page, editor.locator('[data-node-key="transform-object"]'))
    ...
    await clickReady(page, node.locator('[data-testid="key-remove-k1"]'))
  ...
  test('右侧参数逐字编辑不会重建复杂协议画布', async ({ page }) => {
    ...
    // syncReteEditorFromGraph 先 addNode 后 addConnection：toHaveCount(56) 仅说明节点已加完，
    // 连接队列仍在排空，期间 [data-testid="title"] 子树会被反复卸载/重建，dblclick 卡 actionability。
    // 等「≥1 条 stroke≠none、fill=none、width>1 的 path」出现——path 只在 addConnection 后才渲染，
    // 即 sync 已排空、画布静止。与 arrange spec 同款 readiness 信号。
    await expect
      .poll(
    ...
    // 经上 gate 画布已静止；force 兜底绕过 stable 检查的残留抖动，缩短失败时的等待。
    await log.locator('[data-testid="title"]').dblclick({ force: true, timeout: 5000 })
    const drawer = editor.locator('.script-editor-drawer').filter({ hasText: '节点配置' })
    await expect(drawer).toBeVisible()
    // 配置区第一个 textbox 是「前缀」控件输入框。不能再用 drawer.getByRole('textbox').first()：
    // 节点名称编辑功能上线后，header 的 label Input（aria-label="节点名称"）成了抽屉里第一个
    // textbox，会误命中它而非前缀控件。限定到参数 section 内取第一个 textbox。
    const prefix = drawer.locator('.script-editor-inspector__section').getByRole('textbox').first()
    await prefix.focus()
  ...
  test('保存后重开仍保留键与端口（持久化）', async ({ page }) => {
    ...
    await clickReady(page, node.locator('[data-testid="key-add"]'))
    await expect(node.locator('[data-testid="key-name-k1"]')).toBeVisible({ timeout: 10000 })
    await expect(node.locator('[data-testid="input-key_k1"]')).toBeVisible({ timeout: 10000 })
    await node.locator('[data-testid="key-name-k1"]').fill('temperature')
    await expect(node.locator('[data-testid="key-name-k1"]')).toHaveValue('temperature')
